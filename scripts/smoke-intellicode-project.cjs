...
fs.writeFileSync(resultFile,'Starting IntelliCode 1.3 project smoke\n');
...
app.whenReady().then(async()=>{try{
  ...
  const setup=await win.webContents.executeJavaScript(`(async()=>{
    ...
    return {
      version:state?.version,active:state?.active,enabled:state?.enabled,
      projectFiles:state?.projectFiles,projectBytes:state?.projectBytes,
      symbols:state?.symbols,profile:state?.memoryProfile,
      aiConfigured:state?.ai?.configured,aiModel:state?.ai?.model,
      commands:commandIds.every(id=>extensionCommands.has(id))
    };
  })()`);
  log('SETUP',setup);
  if(setup.version!=='1.3.0'||!setup.active||!setup.enabled||setup.projectFiles<3||setup.projectBytes<100||setup.symbols<5||setup.aiConfigured!==false||!setup.commands)throw Error('Project/AI setup did not initialize correctly');
  ...
  const controls=await win.webContents.executeJavaScript(`(async()=>{
    ...
    return {
      ...
      paused:paused.enabled,resumed:resumed.enabled
    };
  })()`);
  ...
  log('PASS project indexing, AI commands registered, controls and unload');
  finish(0);

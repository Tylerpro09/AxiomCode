...
app.whenReady().then(async()=>{try{
  ...
  const result=await win.webContents.executeJavaScript(`(async()=>{
    ...
    const actionIds=['editor.action.revealDefinition','editor.action.peekDefinition','editor.action.goToReferences','editor.action.rename','editor.action.quickFix','editor.action.triggerSuggest','editor.action.quickOutline','editor.action.marker.next'];
  ...
  if(result.caseOnly!==2)throw Error('Case-sensitive search failed');
  if(result.replaced.replacements!==2||result.replaced.filesChanged<1)throw Error('Workspace replace failed');
  if(result.after!==0||result.bars!==2)throw Error('Workspace replace verification failed');
  ...
  if(Object.values(result.actions).some(v=>!v))throw Error('One or more Monaco navigation actions missing');
